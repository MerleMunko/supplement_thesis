...
methods <- c("asymptotic_bonf", "permutation_bonf")
m <- length(methods)
...
for(set.ind in 1:nrow(Settings)){
  ...
  # censoring rates
  save(censp_s, file = paste(path,"/results/bonf/censrate/cens_",set_names[set.ind],".Rdata", sep=""))
  # results for the global error rates
  save(results, file = paste(path, "/results/bonf/errorrate/",set_names[set.ind],".Rdata", sep=""))
  # results for the local error rates
  save(results_multiple, file = paste(path, "/results/bonf/multiple_errorrate/multiple_",set_names[set.ind],".Rdata", sep=""))
  # decision list for all decisions
  save(decision_arr, file = paste(path, "/results/bonf/decision_list/decision_",set_names[set.ind],".Rdata", sep=""))
