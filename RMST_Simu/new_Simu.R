...
n_vec_list       <- list(unbalanced_large  = 4*c(10,20,10,20))
# The survival distribution setting  
distr_vec       <- c("pwExp_diff", "Weib_diff")  
...
for(set.ind in 1:nrow(Settings)){
  ...
  # censoring rates
  save(censp_s, file = paste(path,"/results_new/censrate/cens_",set_names[set.ind],".Rdata", sep=""))
  # results for the global error rates
  save(results, file = paste(path, "/results_new/errorrate/",set_names[set.ind],".Rdata", sep=""))
  # results for the local error rates
  save(results_multiple, file = paste(path, "/results_new/multiple_errorrate/multiple_",set_names[set.ind],".Rdata", sep=""))
  # decision list for all decisions
  save(decision_arr, file = paste(path, "/results_new/decision_list/decision_",set_names[set.ind],".Rdata", sep=""))
